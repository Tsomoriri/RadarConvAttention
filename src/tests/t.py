    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            if normalized_grad[i, j] > dense_threshold:
                ...
                density = (normalized_grad[i, j] - sparse_threshold) / (dense_threshold - sparse_threshold)
                if np.random.rand() < density:
                    updated_grid[i, j] = 1  # Add a point with probability based on density

    return updated_grid



def load_radar_image(file_path='/home/tso/RadarConvAttention/src/datasets/radar_movies.npy'):
    try:
        ...
        if radar_data.ndim > 2:
            ...
            radar_image = radar_data[0, :, :, 2]  # Assuming the first channel is the radar reflectivity
        else:
...
def visualize_grids(radar_image, updated_grid, grid_size=40):
    ...
    if grid_size > radar_height or grid_size > radar_width:
        print(f"Grid size {grid_size} is too large for the radar image dimensions {radar_height}x{radar_width}.")
        return
